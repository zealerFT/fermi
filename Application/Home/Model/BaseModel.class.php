...
class BaseModel extends Model{
     ...
     */
    public function editData($map,$data,$model=''){
        // 去除键值首位空格
        foreach ($data as $k => $v) {
            $data[$k]=trim($v);
        }
     ...
     * @param  integer  $field  指定的字段
     * @return array            想要的数据
     */
    public function getData($model, $where, $page, $limit=10, $order='', $field=''){
        // 获取分页数据
        if (empty($field)) {
            $list=$model
                ->where($where)
