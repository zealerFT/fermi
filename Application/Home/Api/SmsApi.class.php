...
class SmsApi
{
    ...
    public static function qq($token)
    {
        ...
        if ($data['ret'] == 0) {
            ...
            $userInfo['figureurl'] = $data['figureurl'];
            $userInfo['figureurl_1'] = $data['figureurl_1'];
            $userInfo['figureurl_2'] = $data['figureurl_2'];
            $userInfo['figureurl_qq_1'] = $data['figureurl_qq_1'];
            $userInfo['figureurl_qq_2'] = $data['figureurl_qq_2'];
            return $userInfo;
        } else {
    ...
    //登录成功，获取新浪微博用户信息
    public static function weibo($token)
    {
        $weibo = ThinkOauth::getInstance('weibo', $token);
        $data = $weibo->call('users/show', "uid={$weibo->openid()}");
        if ($data['error_code'] == 0) {
            ...
            $userInfo['profile_image_url'] = $data['profile_image_url'];
            $userInfo['avatar_large'] = $data['avatar_large'];
            $userInfo['avatar_hd'] = $data['avatar_hd'];
